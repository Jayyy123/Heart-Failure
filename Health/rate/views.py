...
def database(request):
    e = Details.objects.all()
    return render(request, 'rate/database.html', {'e':e})
...
def loginuser(request):
    if request.method == "POST":
        ...
        user = authenticate(request, username == User.objects.get(username = username), password == User.objects.get(password = password))
...
def result(request,pk):
    ...
    # b.append(request.GET['time'])
    # c = hr.predict([b])
    return render(request, 'rate/result.html', {'c':c})
